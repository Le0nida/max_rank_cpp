...
bool Cell::issingular() const {
    return std::all_of(covered.begin(), covered.end(), [](const HalfSpace& hs) { return hs.arr == Arrangement::SINGULAR; });
}
...
bool Interval::issingular() const {
    return std::all_of(covered.begin(), covered.end(), [](const HalfSpace& hl) { return hl.arr == Arrangement::SINGULAR; });
}
...
std::vector<Cell> searchmincells_lp(const QNode& leaf, const std::vector<std::string>& hamstrings) {
    ...
    for (const auto& hamstr : hamstrings) {
        ...
        model.lp_.num_col_ = dims + 1;
        model.lp_.num_row_ = A_ub_std.size();
        model.lp_.sense_ = ObjSense::kMaximize;

        model.lp_.col_cost_ = c_std;
        model.lp_.col_lower_ = std::vector<double>(dims + 1, 0);
        model.lp_.col_upper_ = std::vector<double>(dims + 1, std::numeric_limits<double>::infinity());

        model.lp_.row_lower_ = std::vector<double>(A_ub_std.size(), -std::numeric_limits<double>::infinity());
        model.lp_.row_upper_ = b_ub_std;

        model.lp_.a_matrix_.start_.resize(dims + 2);
        model.lp_.a_matrix_.index_.resize(dims * A_ub_std.size());
        model.lp_.a_matrix_.value_.resize(dims * A_ub_std.size());

        int index = 0;
        for (int i = 0; i < A_ub_std.size(); ++i) {
            model.lp_.a_matrix_.start_[i] = index;
            for (int j = 0; j < dims + 1; ++j) {
                model.lp_.a_matrix_.index_[index] = j;
                model.lp_.a_matrix_.value_[index] = A_ub_std[i][j];
                index++;
            }
        }
        model.lp_.a_matrix_.start_[A_ub_std.size()] = index;
